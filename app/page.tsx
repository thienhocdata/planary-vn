...
type Task = { id: number; planId: number | null; title: string; note: string; dueDate: string | null; priority: string; timeSlot: TimeSlot; sortOrder: number; completed: boolean; createdAt: string };
...
const timeSlots: Array<{ id: TimeSlot; label: string; range: string; icon: string }> = [
  { id: "morning", label: "Sáng", range: "05:00 – 11:59", icon: "☀" },
  { id: "afternoon", label: "Chiều", range: "12:00 – 17:59", icon: "◐" },
  { id: "evening", label: "Tối", range: "18:00 – 23:59", icon: "☾" },
];
...
export default function Home() {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => typeof window !== "undefined" && window.localStorage.getItem("planary-sidebar-collapsed") === "true");
  ...
  function closeModal() { setModal(null); setEditing(null); setTaskDate(null); setTaskTimeSlot("morning"); }
  ...
  async function submitTask(event: FormEvent<HTMLFormElement>) {
    ...
    try { const values = { title: form.get("title"), note: form.get("note"), planId: Number(form.get("planId")) || null, dueDate: form.get("dueDate"), priority: form.get("priority"), timeSlot: form.get("timeSlot") }; if (editing?.kind === "task") { const data = await update("task", editing.item.id, values); const task = data.task as Task; setTasks((items) => items.map((item) => item.id === task.id ? task : item)); } else { const data = await send({ type: "task", ...values }); setTasks((items) => [data.task as Task, ...items]); } closeModal(); }
  ...
  async function submitBulkTask(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError(""); const form = new FormData(event.currentTarget);
    try { const data = await send({ type: "bulkTask", title: form.get("title"), note: form.get("note"), planId: Number(form.get("planId")) || null, priority: form.get("priority"), timeSlot: form.get("timeSlot"), weekStart: calendarWeekStart, weeks: Number(form.get("weeks")), weekdays: form.getAll("weekdays").map(Number) }); setTasks((items) => [...(data.tasks as Task[]), ...items]); closeModal(); }
  ...
  async function reorderDayTasks(date: string, timeSlot: TimeSlot, sourceId: number, targetId: number) {
    if (sourceId === targetId) return;
    const ordered = tasks.filter((task) => task.dueDate === date && task.timeSlot === timeSlot).sort((left, right) => left.sortOrder - right.sortOrder || left.id - right.id);
    ...
    setTasks((items) => items.map((task) => task.dueDate === date && task.timeSlot === timeSlot ? { ...task, sortOrder: taskIds.indexOf(task.id) } : task));
    try {
      const response = await fetch("/api/data", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "reorder", dueDate: date, timeSlot, taskIds }) });
  ...
  return <main className={`shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className="sidebar">
      <div className="brand"><span>P</span><div><b>planary</b><small>personal system</small></div><button className="sidebar-toggle" aria-label={sidebarCollapsed ? "Mở thanh bên" : "Thu gọn thanh bên"} title={sidebarCollapsed ? "Mở thanh bên" : "Thu gọn thanh bên"} onClick={() => setSidebarCollapsed((value) => { const next = !value; window.localStorage.setItem("planary-sidebar-collapsed", String(next)); return next; })}>{sidebarCollapsed ? "›" : "‹"}</button></div>
      ...
      <div className={`main-inner motion-stage motion-${section}`}>
        ...
        <>
          ...
          {section === "week" && <>
            ...
            <section className="week-board" aria-label="Thời khóa biểu tuần">
              <div className="week-corner">BUỔI</div>
              {weekDates.map((date) => { const note = dayNotes.find((item) => item.noteDate === date); const isToday = date === today; return <header className={`week-day-head ${isToday ? "today" : ""}`} key={date}><span>{isToday ? "HÔM NAY" : new Intl.DateTimeFormat("vi-VN", { weekday: "short" }).format(new Date(`${date}T12:00:00`)).toUpperCase()}</span><strong>{Number(date.slice(8, 10))}</strong><small>THÁNG {Number(date.slice(5, 7))}</small><div><button aria-label={`Thêm việc ngày ${date}`} onClick={() => { setTaskDate(date); setTaskTimeSlot("morning"); setModal("task"); }}>+</button><button className={note?.content ? "has-note" : ""} aria-label={`Ghi chú ngày ${date}`} onClick={() => setOpenDayNote((value) => value === date ? null : date)}>⌑</button></div>{openDayNote === date && <DayNoteEditor date={date} initialValue={note?.content || ""} note={note} onClose={() => setOpenDayNote(null)} onSave={saveDayNote} onDelete={removeDayNote} />}</header>; })}
              {timeSlots.flatMap((slot) => [<div className={`time-row-label ${slot.id}`} key={`${slot.id}-label`}><span>{slot.icon}</span><b>{slot.label}</b><small>{slot.range}</small></div>, ...weekDates.map((date) => { const slotItems = scopedTasks.filter((task) => task.dueDate === date && task.timeSlot === slot.id).sort((left, right) => left.sortOrder - right.sortOrder || left.id - right.id); return <section className={`schedule-cell ${slot.id}`} key={`${slot.id}-${date}`} onDragOver={(event) => event.preventDefault()}>{slotItems.map((task) => <div className={`week-task-wrap ${draggingTaskId === task.id ? "dragging" : ""}`} key={task.id} onDrop={(event) => { event.preventDefault(); const sourceId = Number(event.dataTransfer.getData("text/plain")); if (sourceId) void reorderDayTasks(date, slot.id, sourceId, task.id); setDraggingTaskId(null); }}><button className="week-task-handle" type="button" draggable aria-label={`Kéo để đổi thứ tự ${task.title}`} title="Kéo để đổi thứ tự" onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(task.id)); setDraggingTaskId(task.id); }} onDragEnd={() => setDraggingTaskId(null)}>⋮⋮</button><button className={`week-task ${task.completed ? "done" : ""}`} onClick={() => toggleTask(task)}><i>{task.completed ? "✓" : ""}</i><span>{task.title}<small>{plans.find((plan) => plan.id === task.planId)?.name || "Chung"}</small></span></button><button className="week-task-edit" aria-label={`Sửa ${task.title}`} onClick={() => beginEdit({ kind: "task", item: task })}>⌕</button><button className="week-task-delete" aria-label={`Xóa ${task.title}`} onClick={() => remove("task", task.id)}>×</button></div>)}{!slotItems.length && <button className="schedule-add" onClick={() => { setTaskDate(date); setTaskTimeSlot(slot.id); setModal("task"); }}>+ Lên lịch</button>}</section>; })])}
            </section>
    ...
    {modal === "task" && <ModalFrame title={editing?.kind === "task" ? "Chỉnh sửa việc" : "Lên lịch mới"} eyebrow="CHỌN NGÀY VÀ BUỔI" close={closeModal}><form onSubmit={submitTask}><Field label="Việc bạn sẽ làm"><input name="title" required defaultValue={editing?.kind === "task" ? editing.item.title : ""} placeholder="Ví dụ: Hoàn thành bản nháp đầu tiên" /></Field><Field label="Ghi chú"><textarea name="note" defaultValue={editing?.kind === "task" ? editing.item.note : ""} placeholder="Kết quả mong đợi hoặc bước tiếp theo..." /></Field><div className="form-row"><Field label="Ngày thực hiện"><input name="dueDate" type="date" defaultValue={editing?.kind === "task" ? editing.item.dueDate || "" : taskDate || today} /></Field><Field label="Buổi thực hiện"><select name="timeSlot" defaultValue={editing?.kind === "task" ? editing.item.timeSlot : taskTimeSlot}>{timeSlots.map((slot) => <option value={slot.id} key={slot.id}>{slot.icon} {slot.label} · {slot.range}</option>)}</select></Field></div><div className="form-row"><Field label="Mảng cuộc sống"><PlanSelect plans={plans} selected={editing?.kind === "task" ? editing.item.planId : planFilter} /></Field><Field label="Mức ưu tiên"><select name="priority" defaultValue={editing?.kind === "task" ? editing.item.priority : "normal"}><option value="low">Nhẹ nhàng</option><option value="normal">Bình thường</option><option value="high">Quan trọng</option></select></Field></div><FormActions saving={saving} close={closeModal} label={editing?.kind === "task" ? "Lưu thay đổi" : "Lên lịch"} /></form></ModalFrame>}
