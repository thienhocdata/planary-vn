          ...
          {section === "week" && <>
            ...
            <section className="week-board">{weekDates.map((date) => {
              ...
              return <article className={`day-column ${isToday ? "today" : ""}`} key={date}>
                ...
                <div>{openDayNote === date && <DayNoteEditor date={date} initialValue={note?.content || ""} note={note} onClose={() => setOpenDayNote(null)} onSave={saveDayNote} onDelete={removeDayNote} />}{timeSlots.map((slot) => { const slotItems = items.filter((task) => task.timeSlot === slot.id); return <section className={`day-time-slot ${slot.id}`} key={slot.id}><p><span>{slot.icon} {slot.label}</span><small>{slot.range}</small></p>{slotItems.map((task) => <div className={`week-task-wrap ${draggingTaskId === task.id ? "dragging" : ""}`} key={task.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const sourceId = Number(event.dataTransfer.getData("text/plain")); if (sourceId) void reorderDayTasks(date, slot.id, sourceId, task.id); setDraggingTaskId(null); }}><button className="week-task-handle" type="button" draggable aria-label={`Kéo để đổi thứ tự ${task.title}`} title="Kéo để đổi thứ tự" onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(task.id)); setDraggingTaskId(task.id); }} onDragEnd={() => setDraggingTaskId(null)}>⋮⋮</button><button className={`week-task ${task.completed ? "done" : ""}`} onClick={() => toggleTask(task)}><i>{task.completed ? "✓" : ""}</i><span>{task.title}<small>{plans.find((plan) => plan.id === task.planId)?.name || "Chung"}</small></span></button><button className="week-task-edit" aria-label={`Sửa ${task.title}`} onClick={() => beginEdit({ kind: "task", item: task })}>⌕</button><button className="week-task-delete" aria-label={`Xóa ${task.title}`} onClick={() => remove("task", task.id)}>×</button></div>)}{!slotItems.length && <small className="slot-empty">Chưa có lịch</small>}</section>; })}{!items.length && !note?.content && <p className="free-day">Khoảng thở</p>}</div>
              </article>;
            })}</section>
